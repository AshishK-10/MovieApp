  ...
  next_movies = async ()=> {
    if(this.state.curr_index === this.state.movies.length)
    {
      this.setState({
        ...
        let res = await axios.get(`${process.env.REACT_APP_HEADER}/3/movie/top_rated?api_key=${process.env.REACT_APP_API_KEY}&page=${this.state.curr_page}`);
        let data = res.data
        this.setState({
          movies:[...data.results]
        })
      ...
      <div className = "movie-card">
        {
          ...
          <div id="carouselExampleCaptionsAward" className="carousel slide" data-bs-ride="false">
            ...
            <button className="carousel-control-prev" onClick={this.previous_movies} type="button" data-bs-target="#carouselExampleCaptionsAward" data-bs-slide="prev">
              <span className="carousel-control-prev-icon" aria-hidden="true"></span>
              <span className="visually-hidden">Previous</span>
            </button>

            <button className="carousel-control-next" onClick={this.next_movies} type="button" data-bs-target="#carouselExampleCaptionsAward" data-bs-slide="next">
