          ...
          <div id="carouselExampleCaptionsNewArrivals" className="carousel slide" data-bs-ride="false">
            <div className="carousel-indicators">
              {this.state.movies.map((movie,index) => {
                return (
                  <button type="button" key = {movie.id} data-bs-target="#carouselExampleCaptionsNewArrivals" data-bs-slide-to={index} className={index === 0 ? "active" : ""} aria-current="true" aria-label={index + 1} id = {index + 1} onClick = {()=>{this.update_index(index + 1)}}></button>
                )
              })}
            </div>
